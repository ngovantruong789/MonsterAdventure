...
public partial class SceneLoadView : BaseMonoBehaviour
{
    ...
    {
        _sceneLoadManager.OnCameraChanged
            .Subscribe(val =>
            {
                _parentCanvas.worldCamera = val;
            }).AddTo(this);

        SpawnLoadSceneImages(true);
        DontDestroyOnLoad(transform.parent.gameObject);
    }

    public void ToggleOpenCloseLoadScene(bool isOpen)
    {
        MoveStripes(isOpen);
    }
    ...
    private void MoveStripes(bool isOpen)
    {
        Sequence seq = DOTween.Sequence();

        for (int i = 0; i < _stripes.Count; i++)
        {
            float posX = isOpen ? _stripes[i].openX : _stripes[i].closeX;
            seq.Join(
        ...
        seq.OnComplete(() =>
        {
            _onToggleCompleted.OnNext(isOpen);
        });
    }
}
